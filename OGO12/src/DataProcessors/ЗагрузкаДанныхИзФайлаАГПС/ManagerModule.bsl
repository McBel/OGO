...
//                                           умолчанию - "ЗагрузкаДанныхИзФайлаАГПС").
...
//                                                          параметр, значение по умолчанию - "ЗагрузкаДанныхИзФайлаАГПС").
//     * ОбязательныеКолонкиМакета               - Массив - Содержит список обязательных полей для заполнения.
//     * ЗаголовокКолонкиСопоставления            - Строка - Представление колонки сопоставления в шапке таблицы
//                                                           сопоставления данных(необязательный параметр, значение по
...
Процедура ЗаполнитьТаблицуСопоставленияЗагружаемымиДанными(ШаблонСДанными, ТаблицаИнформацияПоКолонкам, ТаблицаСопоставления, ФоновоеЗадание = Ложь)
	...
	ШаблонСДанными.Записать("D:\Таблица2.mxl");
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Если МетаданныеСправочника.Иерархический Тогда
		...
		Позиция = Позиция + 1;
		Если МетаданныеСправочника.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов тогда		
			СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, "ЭтоГруппа", Позиция);
			Позиция = Позиция + 1;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПоЗагруженнымДаннымИзФайла(ДанныеИзФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	Для каждого Колонка Из ДанныеИзФайла.Колонки Цикл
		...
		КонецЕсли;
		Если НайденнаяКолонка = Неопределено Тогда
			НайденнаяКолонка = НайтиИнформациюОКолонке(ИнформацияПоКолонкам, "ИмяКолонки", СтрокаЗаголовок[Колонка.Имя]);
		КонецЕсли;
		Если НайденнаяКолонка <> Неопределено Тогда
...
Процедура ЗагрузитьФайлВТаблицу(ПараметрыВызоваСервера, АдресХранилища) Экспорт
	...
	ИначеЕсли Расширение = "dbf" Тогда
		ЗагрузитьDBFФайлВТаблицу(ИмяВременногоФайла, ШаблонСДанными, ИнформацияПоКолонкам);
	Иначе
...
#Область РаботаСDBFФайлами

Процедура ЗагрузитьDBFФайлВТаблицу(ИмяФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	ФайлДанных  = Новый XBase(НашПолныйПутьФайла);
  ...
  Для Каждого КолонкаШаблона из ИнформацияПоКолонкам цикл
  		ПолеПроверки = "";
  		Результат = Неопределено;
  		Если ЗначениеЗаполнено(КолонкаШаблона.Синоним) Тогда
  			ПолеПроверки = КолонкаШаблона.Синоним;
  		ИначеЕсли ЗначениеЗаполнено(КолонкаШаблона.Выражение) тогда
  			ПолеПроверки = "";
  			ВыражениеПроверки = КолонкаШаблона.Выражение;
  		Иначе
  			ПолеПроверки = КолонкаШаблона.ПредставлениеКолонки;
  		КонецЕсли;
  		Попытка
  			ФайлДанных.Последняя();
  			Если ПолеПроверки = "" тогда
  				//результат = Вычислить(ВыражениеПроверки);
  				Выполнить(ВыражениеПроверки);
  			иначе
  				результат = ФайлДанных[ПолеПроверки];
  			Конецесли;
  		Исключение
  			Продолжить// TODO:
  		КонецПопытки;
  		НоваяКолонка = Источник.Колонки.Добавить();
		НоваяКолонка.Имя = КолонкаШаблона.ИмяКолонки;
		Если ЗначениеЗаполнено(ПолеПроверки) тогда
			НоваяКолонка.Заголовок = ПолеПроверки;
		иначе
  			НоваяКолонка.Заголовок = ВыражениеПроверки;
  		КонецЕсли;
  КонецЦикла; 
  
 	// Заполнение строк таблицы
    Если (Источник.Колонки.Количество() <> 0) И (ФайлДанных.Последняя()) Тогда
        Пока Истина Цикл
        	Если ФайлДанных.ЗаписьУдалена() тогда продолжить конецесли;
            СтрокаТаб = Источник.Добавить();
            Для Каждого Поле из Источник.Колонки Цикл
  	             НайденнаяКолонка = НайтиИнформациюОКолонке(ИнформацияПоКолонкам, "ИмяКолонки", Поле.Имя);
              	 Если (НайденнаяКолонка <> Неопределено) Тогда
                	Если ЗначениеЗаполнено(НайденнаяКолонка.Синоним) Тогда
  							ПолеПроверки = НайденнаяКолонка.Синоним;
  					ИначеЕсли ЗначениеЗаполнено(НайденнаяКолонка.Выражение) тогда
  							ПолеПроверки = "";
  							ВыражениеПроверки = НайденнаяКолонка.Выражение;
  					Иначе
  							ПолеПроверки = НайденнаяКолонка.ПредставлениеКолонки;
                	КонецЕсли;
                	Если ПолеПроверки = "" тогда
                		Попытка
  						//результат = Вычислить(ВыражениеПроверки);
  					...
  					иначе
  						результат = ФайлДанных[ПолеПроверки];
                		Если (ФайлДанных.Поля[ПолеПроверки].Тип="N") и (ЗначениеЗаполнено(НайденнаяКолонка.ПорядокСортировки))Тогда
  							Повторить = ФайлДанных.Поля[ПолеПроверки].Длина;
  							результат=Прав("000000"+результат,Повторить);
                		КонецЕсли;
                	Конецесли;
                    СтрокаТаб[НайденнаяКолонка.ИмяКолонки] = Строка(результат);
                КонецЕсли;
            КонецЦикла;
            Если Не ФайлДанных.Предыдущая() Тогда Прервать; КонецЕсли;
        КонецЦикла;
    КонецЕсли;
    ФайлДанных.ЗакрытьФайл();
	УдалитьФайлы(НашПолныйПутьФайла);
	СтрокаПорядкаСортировки ="";
	Для Каждого КолонкаШаблона из ИнформацияПоКолонкам цикл
  		Если ЗначениеЗаполнено(КолонкаШаблона.ПорядокСортировки) тогда
  			СтрокаПорядкаСортировки=?(СтрокаПорядкаСортировки="",СтрокаПорядкаСортировки,СтрокаПорядкаСортировки+", ") +КолонкаШаблона.ИмяКолонки;
  		 КонецЕсли;
	 КонецЦикла; 
	 Если СтрокаПорядкаСортировки <> "" тогда
		 Альфа = Новый СравнениеЗначений;
		 Источник.Сортировать(СтрокаПорядкаСортировки,Альфа);
	 КонецЕсли;
  СтрокаТаб = Источник.Вставить(0);
  Для Каждого Поле из Источник.Колонки Цикл
    	СтрокаТаб[Поле.Имя] = Поле.Имя;
  КонецЦикла;
ЗаполнитьТаблицуПоЗагруженнымДаннымИзФайла(Источник, ШаблонСДанными, ИнформацияПоКолонкам);
КонецПроцедуры

Процедура СохранитьТаблицуВDBFФайл(ПутьКФайлу, ИнформацияПоКолонкам) Экспорт
		...
		Попытка
			...
			Иначе
				Если СтрокаТаблицы.ЭтоГруппа тогда
					ЭлементСправочника = МенеджерСправочника.СоздатьГруппу();
				Иначе
			...
			Для каждого Колонка Из ИнформацияПоКолонкам Цикл 
				Если Колонка.Видимость и Колонка.ИмяКолонки <> "ЭтоГруппа"  Тогда
