...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		ИмяОбъектаСопоставления = Обработки.ЗагрузкаДанныхИзФайлаАГПС.ПолноеИмяОбъектаТабличнаяЧасть(Параметры.ПолноеИмяТабличнойЧасти);
		
		ТаблицаИнформацияПоКолонкам = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЗагрузкаДанныхИзФайлаАГПС", ИмяОбъектаСопоставления,, ИмяПользователя());
		...
		Иначе
			Обработки.ЗагрузкаДанныхИзФайлаАГПС.ОпределитьИнформацияПоКолонкам(ПараметрыЗагрузки, ТаблицаИнформацияПоКолонкам);
...
Процедура СписокКолонокСопоставленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайлаАГПС.Форма.ВыборКолонок", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗагрузитьШаблонИзФайла(Команда)
	...
	ЗагрузкаДанныхИзФайлаАГПСКлиент.ДиалогЗагрузкиФайла(Оповещение, ИмяФайла);
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайлаАГПС.Форма.РасширениеФайла",, ЭтотОбъект, Истина,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Обработки.ЗагрузкаДанныхИзФайлаАГПС.УстановитьРежимВставкиИзБуфераОбмена(ШаблонСДанными, ИнформацияПоКолонкам, Параметры.ОписаниеТипов);
...
Процедура ЗаполнитьСписокВидЗагрузкиДанных()
	Обработки.ЗагрузкаДанныхИзФайлаАГПС.СоздатьСписокСправочниковДляЗагрузки(СписокВариантовЗагрузки);
КонецПроцедуры 
...
Процедура ОткрытьФормуИзмененияБланка()
	...
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайлаАГПС.Форма.РедактированиеБланка", ПараметрыФормы, ЭтотОбъект,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
		ПараметрыЗагрузки.Вставить("ИмяМакетаСШаблоном", "ЗагрузкаДанныхИзФайлаАГПС");
		Обработки.ЗагрузкаДанныхИзФайлаАГПС.ПараметрыЗагрузкиИзФайлаВнешняяОбработка(ИдентификаторКоманды,
			ВнешняяОбработкаСсылка, ПараметрыЗагрузки);
	КонецЕсли;
	ПараметрыЗагрузки.Вставить("ТипЗагрузки", ТипЗагрузки);
	ПараметрыЗагрузки.Вставить("ПолноеИмяОбъекта", ИмяОбъектаСопоставления);
	
	ИнформацияПоКолонкамТаблица = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЗагрузкаДанныхИзФайлаАГПС", ИмяОбъектаСопоставления,, ИмяПользователя());
	...
	КонецЕсли;
	Обработки.ЗагрузкаДанныхИзФайлаАГПС.ОпределитьИнформацияПоКолонкам(ПараметрыЗагрузки, ИнформацияПоКолонкамТаблица);
...
Процедура СохранитьТаблицуВCSVФайл(ПолноеИмяФайла)
	Обработки.ЗагрузкаДанныхИзФайлаАГПС.СохранитьТаблицуВCSVФайл(ПолноеИмяФайла, ИнформацияПоКолонкам);
КонецПроцедуры
...
Процедура СопоставитьЗагружаемыеДанныеНаСервере()
	...
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		...
		Обработки.ЗагрузкаДанныхИзФайлаАГПС.ТабличныйДокументВТаблицуЗначений(ШаблонСДанными, ТаблицаИнформацияПоКолонкам, АдресЗагруженныхДанных);
	...
	ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Обработки.ЗагрузкаДанныхИзФайлаАГПС.ЗаполнитьТаблицуСопоставленияДаннымиИзШаблона(ШаблонСДанными, ТаблицаСопоставления, ИнформацияПоКолонкам);
		Обработки.ЗагрузкаДанныхИзФайлаАГПС.СопоставитьЗначениеКолонкиАвто(ТаблицаСопоставления, "Ссылки");
		...
		Если Строка.РезультатСопоставленияСтроки = "Неоднозначность" И СтрДлина(Строка.ОписаниеОшибки) > 0 Тогда
			Если СтрДлина(ПолеИмя) > 3 И СтрНачинаетсяС(ПолеИмя, "ТаблицаСопоставленияДанных_ТЧ_") Тогда
				...
				Если СтрНайти(Строка.ОписаниеОшибки, Имя) Тогда
					...
					ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайлаАГПС.Форма.РазрешенияНеоднозначностей", ПараметрыФормы, ЭтотОбъект, Истина , , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Процедура ВыполнитьСопоставлениеПоВыбранномуРеквизиту(КоличествоСопоставленных = 0, СписокКолонокСопоставления = "")
	...
	СтруктураОбъекта = Обработки.ЗагрузкаДанныхИзФайлаАГПС.РазложитьПолноеИмяОбъекта(ИмяОбъектаСопоставления);
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		МассивОбъекта = Обработки.ЗагрузкаДанныхИзФайлаАГПС.РазложитьПолноеИмяОбъекта(ИмяОбъектаСопоставления);
...
Функция ОписаниеТипаПоМетаданным(ПолноеИмяОбъектаМетаданных)
	Результат = Обработки.ЗагрузкаДанныхИзФайлаАГПС.РазложитьПолноеИмяОбъекта(ПолноеИмяОбъектаМетаданных);
...
Процедура ОпределитьПараметрыЗагрузки(ПараметрыЗагрузкиИзФайла)
	...
	ПараметрыЗагрузкиИзФайла = Обработки.ЗагрузкаДанныхИзФайлаАГПС.ПараметрыЗагрузкиИзФайла(МетаданныеОбъекта);
...
Процедура ИзменитьБланкПоИнформацииПоКолонкам(Бланк = Неопределено, СохранитьНастройки = Ложь)
	...
	Если СохранитьНастройки Тогда
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ЗагрузкаДанныхИзФайлаАГПС", ИмяОбъектаСопоставления, ТаблицаКолонок,, ИмяПользователя());
	КонецЕсли;
	
	Бланк.Очистить();
	Шапка = Обработки.ЗагрузкаДанныхИзФайлаАГПС.ШапкаБланкаДляЗаполненияПоИнформацииПоКолонкам(ТаблицаКолонок);
	Бланк.Вывести(Шапка);
	ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки();
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайлаАГПС.ЗагрузитьФайлВТаблицу",
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайлаАГПС.ЗаполнитьТаблицуСопоставленияДаннымиИзШаблонаФон", 
		ПараметрыВызоваСервера, ПараметрыВыполненияВФоне);
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайлаАГПС.ЗаписатьСопоставленныеДанные", 
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайлаАГПС.СформироватьОтчетОЗагрузкеФон",
		ПараметрыВызоваСервера, ПараметрыВыполнения);
